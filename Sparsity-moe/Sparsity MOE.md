...
|  |  |
| **Gating Mechanism 1** | Gate is **not updated by Backprop** from MSE, instead from **RC Loss** After main backward pass, each Gate looks at loss  reaching each selected Expert. If loss exceeds a **quantile q compared to other gates***(This is a **hyperparameter** that can be set, shd be worst 70%)* of loss  , output of selected Expert is deemed **incorrect**. Target is set to **0**. All other **unselected** Experts have Target set to N/E. Unselected means neither correct nor wrong. 1Target\>0 *N \= Number of incorrectly-chosen Experts E \= Number of unselected Experts*  Weight for each Expert is updated by **BCE** loss, of value to Target *(EG. Expert is selected, but wrong. BCE of 0.4 vs 0, where 0.4 is forward weight for expert at pixel.) (EG. Expert is not selected, but wrong. BCE of 0 vs 0.2, where 0 is forward weight for expert at pixel.) (EG. Expert is selected & correct. BCE of 0.4 vs 1, where 0.4 is forward weight for expert at pixel.)*  This is done **independently** for each pixel of grid map.  |
